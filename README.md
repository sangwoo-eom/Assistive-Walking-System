...
│   ├── model_manager.py     # 모델 로딩 및 추론 파이프라인
│   ├── risk.py              # 객체 위험도 계산 (TTC, 접근성)
│   ├── env_risk.py          # 환경 영역 위험 판단
│   ├── warning.py           # 경고 상태 머신
│   ├── stt.py               # Whisper 기반 음성 인식
│   ├── tts.py               # 음성 출력 문장 생성
│   ├── location_identity.py # 위치 기반 식별 로직
│   └── utils.py             # 공통 유틸 함수
│
├── models/                  # AI 모델 래퍼
...
│
├── routes/                  # FastAPI 라우터
│   ├── inference.py         # 실시간 추론 API
│   ├── identity.py          # 위치 기반 API
│   └── stt.py               # 음성 인식 API
│
├── static/                  # 프론트엔드 리소스
│   ├── css/
