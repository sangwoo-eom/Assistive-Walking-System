...
def convert_to_yolo(size, box):
    """Convert VOC-style bbox to YOLO format"""
    W, H = size
...
def load_classes(root):
    """Scan dataset and collect all class labels"""
    classes = set()
...
def export_all_frames(src_root, class_to_id):
    """Export all frames and annotations to YOLO format"""
    ...
    for folder in tqdm(sorted(os.listdir(src_root)), desc="Exporting all frames"):
        ...
        for image in root.findall("image"):
            ...
            with open(
                os.path.join(LBL_OUT, out_name.replace(".jpg", ".txt")), "w"
            ) as f:
                f.write("\n".join(lines))
...
print("\nStep 1. Loading class list...")
class_list, class_to_id = load_classes(SRC)

print("\nStep 2. Exporting all frames to YOLO...")
